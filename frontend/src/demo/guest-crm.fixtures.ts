...
const casa = [guest("familia-casa", "Família Oliveira", "Família", ["Família", "Airbnb e curta temporada"], 2, 2400), guest("carol-casa", "Carolina e André", "Férias", ["Casais"], 1, 1300), guest("bruno-casa", "Bruno Teixeira", "Férias", ["Airbnb e curta temporada"], 2, 1900, { digital: ["Portal acessado", "QR Code: utilizado", "Automação Tuya: utilizada", "Código de acesso: ••••"] })];

export function getGuestCrmDemo(organizationName?: string | null, organizationId?: string) { const key = (organizationName ?? "").normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase(); if (key.includes("monaco") || key.includes("summit")) return monaco; if (key.includes("vila nova") || key.includes("studio")) return [...getManualCrmGuests(organizationId), ...studio]; if (key.includes("mairipora")) return casa; return []; }
