...
const steps = [{ title: "Empresa cliente", description: "Dados da empresa", icon: Building2 }, { title: "Propriedade", description: "Primeira propriedade", icon: Hotel }, { title: "Automação", description: "Recursos inteligentes", icon: Sparkles }, { title: "Unidade", description: "Primeira unidade", icon: DoorOpen }];
...
export function OnboardingPage() {
  ...
  return <main className="min-h-screen bg-background"><header className="border-b"><div className="mx-auto flex h-[72px] max-w-[1180px] items-center justify-between px-5"><BrandMark /><Badge variant="outline">{criandoNovaEmpresa ? "Nova empresa cliente" : "Configuração inicial"}</Badge></div></header><div className="mx-auto grid max-w-[1180px] gap-8 px-5 py-8 lg:grid-cols-[260px_1fr] lg:py-12">
    ...
    <Card><CardHeader><CardTitle>{steps[step].title}</CardTitle><CardDescription>{step === 0 ? "Cadastre os dados da empresa cliente no Essencial Stay." : step === 1 ? "Adicione a primeira propriedade. Outras poderão ser incluídas depois." : step === 2 ? "Informe os recursos inteligentes existentes. A conexão com fabricantes pode ser feita depois." : "Crie a primeira unidade da propriedade."}</CardDescription></CardHeader><CardContent><form className="space-y-5" onSubmit={form.handleSubmit(submit)}>
