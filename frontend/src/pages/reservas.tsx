...
function ManualExperienceCard({ reservation, onClose, onUpdate, onSuccess, onDemoCompleted, accessOpen, onAccessOpenChange }: {
    ...
    function completePreCheckin() { updateManualAirbnbPreparation(reservation.id, { preCheckinCompleted: true }, [reservation.lockMode === "automatic-demo" ? "Pré-check-in iniciado" : "Pré-check-in concluído"]); onUpdate(); showToast("Pré-check-in concluído."); if (!requiresFNRH && reservation.lockMode === "automatic-demo") void prepareAccess(); }
    ...
    const journeySteps = ["Reserva", "Pré-check-in", ...(requiresFNRH ? ["FNRH"] : []), isYale ? "PIN Yale" : "Acesso automático", "Portal", "QR", "Mensagem", "Envio"];
    const completionItems = ["Reserva confirmada", "Pré-check-in concluído", ...(requiresFNRH ? ["FNRH registrada"] : []), `Acesso ${isYale ? "Yale" : "Ekaza"} preparado`, "Portal do Hóspede criado", "QR Code criado", "Mensagem preparada"];
...
function DemoSuccessCard({ type, onClose }: { type: DemoSuccessState; onClose: () => void }) {
    ...
    const items = preparation ? ["Reserva confirmada", "Portal criado", "QR Code preparado", "PIN configurado", "Mensagem pronta"] : [];
