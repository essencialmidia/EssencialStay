...
import { addManualAirbnbReservation, clearManualAirbnbReservations, endGuidedDemoReservation, getGuidedDemoReservation, getManualAirbnbReservations, removeManualAirbnbReservation, type ManualAirbnbReservation } from "../demo/manual-airbnb-reservations";
...
export function ReservasPage() {
  ...
  const [guidedModal, setGuidedModal] = useState<{ reservationId: string; type: "checkin" | "checkout" } | null>(null);
  ...
  useEffect(() => { setGuidedModal(null); setManualReservation(null); }, [organizacaoAtual?.id]);
  const summary = [
  ...
  return (
    <div className="space-y-7">
      ...
      {isVilaNova && getGuidedDemoReservation(organizacaoAtual?.id) && <GuidedDemoPanel reservation={getGuidedDemoReservation(organizacaoAtual?.id)!} onUpdate={() => setManualReservation((current) => current ? { ...current } : current)} onOpenCheckin={() => { const reservation = getGuidedDemoReservation(organizacaoAtual?.id); if (!reservation) return; setManualReservation(reservation); setGuidedModal({ reservationId: reservation.id, type: "checkin" }); showToast("Check-in preparado."); }} onOpenCheckout={() => { const reservation = getGuidedDemoReservation(organizacaoAtual?.id); if (!reservation) return; setManualReservation(reservation); setGuidedModal({ reservationId: reservation.id, type: "checkout" }); showToast("Checkout preparado."); }} onEndKeep={() => { const reservation = getGuidedDemoReservation(organizacaoAtual?.id); if (!reservation) return; endGuidedDemoReservation(reservation.id); setGuidedModal(null); setManualReservation({ ...reservation, isGuidedDemo: false }); showToast("Demonstração encerrada. A reserva foi mantida."); }} onEndRemove={() => { const reservation = getGuidedDemoReservation(organizacaoAtual?.id); if (!reservation) return; removeManualAirbnbReservation(reservation.id); setGuidedModal(null); setManualReservation(null); showToast("Demonstração encerrada e reserva removida."); }} />}
...
function AirbnbReservationModal({ open, organizationId, onClose, onCreated }: { open: boolean; organizationId: string; onClose: () => void; onCreated: (reservation: ManualAirbnbReservation) => void }) {
  ...
  function create() { const id = `demo-${crypto.randomUUID().replace(/-/g, "").slice(0, 12)}`; onCreated({ id, organizationId, guestName: name || "Hóspede demonstrativo", phone: phone || "+55 (11) 9••••-••••", checkIn, checkOut, guests: Number(guests), notes, source, portalUrl: getDemoPublicUrl(`/s/vila-nova-demo?reservaId=${id}`), pinMasked: "48••15", status: "Reserva criada", timeline: ["Reserva criada", "Portal criado", "Concierge criado", "PIN preparado", "QR gerado"] }); setStep(1); setName(""); setPhone(""); }
...
function GuidedDemoModal({ open, organizationId, existing, onClose, onSelect }: { open: boolean; organizationId: string; existing: ManualAirbnbReservation | null; onClose: () => void; onSelect: (reservation: ManualAirbnbReservation) => void }) {
  function create(restart = false) { if (restart && existing) removeManualAirbnbReservation(existing.id); const id = `demo-${crypto.randomUUID().replace(/-/g, "").slice(0, 12)}`; const checkIn = new Date(); checkIn.setDate(checkIn.getDate() + 1); checkIn.setHours(15, 0, 0, 0); const checkOut = new Date(checkIn); checkOut.setDate(checkOut.getDate() + 2); const reservation: ManualAirbnbReservation = { id, organizationId, guestName: "Mariana Oliveira", phone: "+55 (11) 90000-0000", checkIn: checkIn.toISOString().slice(0, 16), checkOut: checkOut.toISOString().slice(0, 16), guests: 2, notes: "Reserva criada pelo Modo Demonstração Essencial Stay.", source: "Airbnb", portalUrl: getDemoPublicUrl(`/s/vila-nova-demo?reservaId=${id}`), pinMasked: "48••15", status: "Reserva criada", timeline: ["Reserva criada", "Portal criado", "Concierge criado", "PIN preparado", "QR gerado"], isGuidedDemo: true, demoSessionId: crypto.randomUUID(), demoCreatedAt: new Date().toISOString() }; addManualAirbnbReservation(reservation); onSelect(reservation); }
  return <Modal open={open} title="Demonstração da jornada Essencial Stay" description="Uma reserva fictícia será preparada para apresentar o fluxo completo da hospedagem. Nenhuma mensagem, PIN ou integração real será acionada." onClose={onClose}><div className="space-y-4"><ol className="list-decimal space-y-1 pl-5 text-sm text-muted-foreground"><li>Reserva rápida</li><li>Link e portal do hóspede</li><li>Concierge Digital</li><li>Check-in, hospedagem e checkout</li><li>Limpeza e Histórico CRM</li></ol>{existing ? <div className="flex flex-col gap-2"><Button onClick={() => onSelect(existing)}>Continuar demonstração existente</Button><Button variant="outline" onClick={() => create(true)}>Reiniciar demonstração</Button></div> : <Button onClick={() => create()}>Começar nova demonstração</Button>}<Button variant="ghost" className="w-full" onClick={onClose}>Cancelar</Button></div></Modal>;
}
