import { ArrowRight, CalendarCheck, CalendarClock, Check, CheckCircle2, Clock3, Copy, ExternalLink, Filter, Hotel, House, KeyRound, Mail, MessageCircle, NotebookPen, Plus, Search, Send, Sparkles, TriangleAlert, UserRound, UsersRound, X, type LucideIcon } from "lucide-react";
...
type DemoSuccessState = "preparation" | "message";
const tabs: Array<{
    value: StayTab;
    label: string;
}> = [
...
export function ReservasPage() {
    ...
    useEffect(() => { if (!demoSuccess) return; const timeout = window.setTimeout(() => setDemoSuccess(null), 5200); return () => window.clearTimeout(timeout); }, [demoSuccess]);
    const summary = [
    ...
    return (<div className="space-y-7">
      ...
      {demoSuccess && <DemoSuccessCard type={demoSuccess} onClose={() => setDemoSuccess(null)}/>}
...
function ManualExperienceCard({ reservation, onClose, onUpdate, onSuccess, onDemoCompleted, accessOpen, onAccessOpenChange }: {
    ...
    function sendExperience() { if (!getDemoAccessSecret(reservation.id)) {
        showToast("O acesso temporário ainda não foi preparado.");
        return;
    } sendManualAirbnbExperience(reservation.id); onUpdate(); setSendOpen(false); setAccessOpen(false); onDemoCompleted?.(); onSuccess("message"); }
