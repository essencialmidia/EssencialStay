...
function ManualExperienceCard({ reservation, onClose, onUpdate, onSuccess, onDemoCompleted, accessOpen, onAccessOpenChange }: {
    ...
    const isStudioVilaNova = reservation.unitId === "studio-vila-nova";
    ...
    function confirmYalePin() { const cleanPin = yalePin.replace(/\D/g, ""); if (!cleanPin)
        return; setIsConfirmingPin(true); window.setTimeout(() => { setDemoAccessSecret(reservation.id, cleanPin); updateManualAirbnbAccess(reservation.id, { pinMasked: `${cleanPin.slice(0, 2)}${"•".repeat(Math.max(4, cleanPin.length - 2))}`, accessStatus: "active", accessPrepared: true }, ["Portal atualizado", "QR Code criado", "Mensagem preparada"]); setYalePin(""); setIsConfirmingPin(false); setAccessOpen(false); onUpdate(); onSuccess("preparation"); }, 650); }
    ...
    const checklist = isStudioVilaNova
        ? [["Reserva confirmada", true], ["PIN Yale", Boolean(reservation.accessPrepared)], ["QR Code", Boolean(reservation.accessPrepared)], ["Portal do Hóspede", Boolean(reservation.accessPrepared)], ["Mensagem", Boolean(reservation.accessPrepared)]] as const
    ...
    const nextStep = reservation.sent ? "🎉 Hospedagem preparada e enviada ao hóspede" : !reservation.preCheckinCompleted ? isStudioVilaNova ? "Preparar PIN Yale" : "Iniciar pré-check-in" : requiresFNRH && !reservation.fnrhCompleted ? "Concluir FNRH Digital" : isYale && !reservation.accessPrepared ? "Aguardando PIN Yale" : reservation.accessStatus === "generating" ? "Gerando acesso automaticamente" : "Confirmar envio ao hóspede";
    const journeySteps = isStudioVilaNova ? ["Reserva confirmada", "PIN Yale", "QR Code", "Portal do Hóspede", "Mensagem"] : ["Reserva", "Pré-check-in", ...(requiresFNRH ? ["FNRH"] : []), isYale ? "PIN Yale" : "Acesso automático", "Portal", "QR", "Mensagem", "Envio"];
    const completionItems = isStudioVilaNova ? ["Reserva confirmada", "PIN Yale configurado", "QR Code preparado", "Portal do Hóspede criado", "Mensagem pronta"] : ["Reserva confirmada", "Pré-check-in concluído", ...(requiresFNRH ? ["FNRH registrada"] : []), `Acesso ${isYale ? "Yale" : "Ekaza"} preparado`, "Portal do Hóspede criado", "QR Code criado", "Mensagem preparada"];
