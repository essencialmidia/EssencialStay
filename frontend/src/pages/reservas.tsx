...
import { addManualAirbnbReservation, clearManualAirbnbReservations, getGuidedDemoReservation, getManualAirbnbReservations, removeManualAirbnbReservation, type ManualAirbnbReservation } from "../demo/manual-airbnb-reservations";
...
export function ReservasPage() {
  ...
  return (
    <div className="space-y-7">
      ...
      {isVilaNova && getGuidedDemoReservation(organizacaoAtual?.id) && <GuidedDemoPanel reservation={getGuidedDemoReservation(organizacaoAtual?.id)!} onUpdate={() => setManualReservation((current) => current ? { ...current } : current)} onOpenCheckin={() => showToast("Use o modal de check-in no cartão Operação da hospedagem.")} onOpenCheckout={() => showToast("Use o modal de checkout no cartão Operação da hospedagem.")} />}
