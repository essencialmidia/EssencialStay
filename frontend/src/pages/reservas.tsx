import { ArrowRight, CalendarCheck, CalendarClock, Check, CheckCircle2, Clock3, Copy, ExternalLink, Filter, Hotel, House, KeyRound, Mail, MapPin, MessageCircle, NotebookPen, Plus, QrCode, Search, Send, ShieldCheck, Sparkles, TriangleAlert, UserRound, UsersRound, X, type LucideIcon } from "lucide-react";
...
import { getStudioVilaNovaJourney, isStudioVilaNovaJourney } from "../demo/studio-vila-nova-journey";
import { DemoAutomaticAccessProvider, demoLockConfigurations, demoReservationRequiresFNRH } from "../demo/lock-access-demo";
...
type DemoSuccessState = "preparation" | "message" | "studio-message";
const tabs: Array<{
    value: StayTab;
    label: string;
}> = [
...
export function ReservasPage() {
    ...
    useEffect(() => { if (!demoSuccess || demoSuccess === "studio-message") return; const timeout = window.setTimeout(() => setDemoSuccess(null), 5200); return () => window.clearTimeout(timeout); }, [demoSuccess]);
    const summary = [
    ...
    return (<div className="space-y-7">
      ...
      {isVilaNova && <GuidedDemoModal open={demoOpen} organizationId={organizacaoAtual?.id || ""} existing={getGuidedDemoReservation(organizacaoAtual?.id)} onClose={() => setDemoOpen(false)} onSelect={(reservation) => { if (isStudioVilaNovaJourney(reservation.unitId) && !reservation.preCheckinCompleted) updateManualAirbnbPreparation(reservation.id, { preCheckinCompleted: true }, ["Preparação interna concluída"]); setManualReservation(reservation); setDemoOpen(false); setGuidedAccessReservationId(isStudioVilaNovaJourney(reservation.unitId) && reservation.lockProvider === "yale" && !reservation.accessPrepared ? reservation.id : null); }}/>}
      {demoSuccess && <DemoSuccessFeedback type={demoSuccess} onClose={() => { if (demoSuccess === "studio-message") { const reservation = getGuidedDemoReservation(organizacaoAtual?.id); if (reservation) endGuidedDemoReservation(reservation.id); setGuidedAccessReservationId(null); } setDemoSuccess(null); }}/>}
      {manualReservation && <ManualExperienceCard reservation={manualReservation} onClose={() => setManualReservation(null)} onUpdate={() => setManualReservation((current) => current ? { ...current } : current)} onSuccess={setDemoSuccess} onDemoCompleted={() => { if (manualReservation.isGuidedDemo)
      ...
      {isVilaNova && !manualReservation && manualReservations.map((reservation) => <ManualExperienceCard key={reservation.id} reservation={reservation} onClose={() => setManualReservation(null)} onUpdate={() => setManualReservation((current) => current ? { ...current } : current)} onSuccess={setDemoSuccess} onDemoCompleted={() => { if (reservation.isGuidedDemo)
...
function DemoFieldLabel({ icon: Icon, children }: { icon: LucideIcon; children: React.ReactNode }) {
    return <span className="inline-flex items-center gap-2"><Icon className="size-4 text-muted-foreground" aria-hidden="true"/>{children}</span>;
}
function BrazilianDateTimeField({ label, value, onChange, rangeError }: { label: string; value: string; onChange: (value: string) => void; rangeError?: string }) {
    const initial = isoLocalToVisual(value);
    const [dateValue, setDateValue] = useState(initial.date);
    const [timeValue, setTimeValue] = useState(initial.time);
    const [touched, setTouched] = useState(false);
    useEffect(() => { if (value) { const next = isoLocalToVisual(value); setDateValue(next.date); setTimeValue(next.time); } }, [value]);
    const dateValid = isValidBrazilianDate(dateValue);
    const timeValid = isValid24HourTime(timeValue);
    function commit(date: string, time: string) { onChange(isValidBrazilianDate(date) && isValid24HourTime(time) ? visualDateTimeToIsoLocal(date, time) : ""); }
    return <fieldset className="rounded-xl border bg-surface/50 p-4">
        <legend className="px-1 text-sm font-semibold"><DemoFieldLabel icon={CalendarCheck}>{label}</DemoFieldLabel></legend>
        <div className="mt-1 grid gap-3 sm:grid-cols-2">
            <FormField label="Data" error={touched && !dateValid ? "Informe uma data válida." : undefined}><Input value={dateValue} onChange={(event) => { const next = formatBrazilianDateInput(event.target.value); setDateValue(next); commit(next, timeValue); }} onBlur={() => setTouched(true)} inputMode="numeric" placeholder="DD/MM/AAAA" maxLength={10} aria-invalid={touched && !dateValid}/></FormField>
            <FormField label="Hora" error={touched && !timeValid ? "Informe um horário válido." : undefined}><Input value={timeValue} onChange={(event) => { const next = format24HourTimeInput(event.target.value); setTimeValue(next); commit(dateValue, next); }} onBlur={() => setTouched(true)} inputMode="numeric" placeholder="HH:mm" maxLength={5} aria-invalid={touched && !timeValid}/></FormField>
        </div>
        {rangeError && <p className="mt-2 text-xs text-destructive" role="alert">{rangeError}</p>}
    </fieldset>;
}
function AirbnbReservationModal({ open, organizationId, onClose, onCreated }: {
    ...
    function create() { const id = `demo-${crypto.randomUUID().replace(/-/g, "").slice(0, 12)}`; const guestName = name || "Hóspede demonstrativo"; const { slug, url } = vilaNovaPortalUrl(id, organizationId, guestName); onCreated({ id, organizationId, guestName, phone, phoneNormalized: normalizeBrazilianPhone(phone), checkIn, checkOut, guests: Number(guests), notes, source, portalUrl: url, portalSlug: slug, pinMasked: "••••••", unitId, unitName: lock.unitName, lockProvider: lock.provider, lockMode: lock.mode, requiresFNRH: lock.requiresFNRH, accessStatus: lock.mode === "assisted" ? "awaiting_manual_pin" : undefined, accessPrepared: false, status: "Reserva criada", timeline: lock.mode === "assisted" ? ["Reserva confirmada", "Unidade definida"] : ["Reserva criada", "Unidade definida"] }); setStep(1); setName(""); setPhone(""); }
    return <Modal open={open} title="Preparar nova hospedagem" description="Preencha as informações da reserva para iniciar a jornada do hóspede." onClose={onClose}>{step === 1 ? <div className="space-y-3"><FormField label={<DemoFieldLabel icon={UserRound}>Nome do hóspede *</DemoFieldLabel>}><Input value={name} onChange={(event) => setName(event.target.value)} autoFocus placeholder="Ex.: Mariana Oliveira"/></FormField><FormField label={<DemoFieldLabel icon={MessageCircle}>WhatsApp do hóspede *</DemoFieldLabel>}><Input value={phone} onChange={(event) => setPhone(formatBrazilianPhone(event.target.value))} inputMode="tel" placeholder="Ex.: (11) 98765-4321"/>{phone && !phoneValid && <p className="mt-1 text-xs text-destructive">Informe um celular com DDD.</p>}</FormField><FormField label={<DemoFieldLabel icon={House}>Unidade</DemoFieldLabel>}><Select value={unitId} onChange={(event) => setUnitId(event.target.value)}>{Object.values(demoLockConfigurations).map((item) => <option key={item.id} value={item.id}>{item.unitName}</option>)}</Select></FormField><div className="rounded-md border bg-surface p-3 text-sm"><p className="font-semibold">{lock.displayName} — {lock.mode === "assisted" ? "modo assistido" : "modo demonstração"}</p>{lock.mode === "assisted" ? <p className="mt-1 text-muted-foreground">O PIN Yale será informado após o pré-check-in{lock.requiresFNRH ? " e a FNRH" : ""}. Nenhuma integração Yale é usada.</p> : <p className="mt-1 text-muted-foreground">O acesso será gerado por um provider local de demonstração.</p>}</div><div className="grid gap-3 sm:grid-cols-2"><BrazilianDateTimeField label="Check-in" value={checkIn} onChange={setCheckIn}/><BrazilianDateTimeField label="Check-out" value={checkOut} onChange={setCheckOut} rangeError={checkIn && checkOut && !isCheckoutAfterCheckin(checkIn, checkOut) ? "O check-out deve ser posterior ao check-in." : undefined}/></div><div className="grid grid-cols-2 gap-3"><FormField label={<DemoFieldLabel icon={UsersRound}>Hóspedes</DemoFieldLabel>}><Input type="number" min="1" value={guests} onChange={(event) => setGuests(event.target.value)}/></FormField><FormField label="Origem"><Select value={source} onChange={(event) => setSource(event.target.value as ManualAirbnbReservation["source"])}><option>Airbnb</option><option>Booking</option><option>Direta</option><option>Outro</option></Select></FormField></div><FormField label={<DemoFieldLabel icon={NotebookPen}>Observações</DemoFieldLabel>}><Input value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Ex.: Chegada após 22h ou necessita berço"/></FormField><Button className="w-full" disabled={!name || !phoneValid || !isCheckoutAfterCheckin(checkIn, checkOut)} onClick={() => setStep(2)}>Continuar →</Button></div> : <div className="space-y-4"><div className="rounded-md bg-surface p-4 text-sm"><p className="font-semibold">{name} · {lock.unitName}</p><p>{nights} noites · {source}</p><p>{formatLocalDate(checkIn)} → {formatLocalDate(checkOut)}</p></div><Button className="w-full" onClick={create}>Preparar hospedagem</Button><Button className="w-full" variant="ghost" onClick={() => setStep(1)}>Voltar</Button></div>}</Modal>;
}
function GuidedDemoModal({ open, organizationId, existing, onClose, onSelect }: {
    ...
    const phoneValid = isValidBrazilianMobile(phone);
    function create() { const lock = demoLockConfigurations[unitId]; const id = `demo-${crypto.randomUUID().replace(/-/g, "").slice(0, 12)}`; const { slug, url } = vilaNovaPortalUrl(id, organizationId, guestName); const reservation: ManualAirbnbReservation = { id, organizationId, guestName, phone, phoneNormalized: normalizeBrazilianPhone(phone), checkIn, checkOut, guests: Number(guests) || 1, notes, source: "Airbnb", portalUrl: url, portalSlug: slug, pinMasked: "••••••", unitId, unitName: lock.unitName, lockProvider: lock.provider, lockMode: lock.mode, requiresFNRH: lock.requiresFNRH, accessStatus: lock.mode === "assisted" ? "awaiting_manual_pin" : undefined, accessPrepared: false, status: "Reserva criada", timeline: lock.mode === "assisted" ? ["Reserva confirmada", "Unidade definida"] : ["Reserva criada", "Unidade definida"], isGuidedDemo: true, demoSessionId: crypto.randomUUID(), demoCreatedAt: new Date().toISOString() }; addManualAirbnbReservation(reservation); onSelect(reservation); }
    const stepTitles = ["Dados da hospedagem", "Escolha da fechadura", "Resumo"];
    return <Modal open={open} title="Preparar nova hospedagem" description="Preencha as informações da reserva para iniciar a jornada do hóspede." onClose={onClose} size="large"><div className="mx-auto max-w-3xl space-y-7"><div className="flex items-center gap-2">{stepTitles.map((title, index) => <div key={title} className="flex flex-1 items-center gap-2"><div className={`flex size-8 shrink-0 items-center justify-center rounded-full text-sm font-semibold ${step >= index + 1 ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}>{index + 1}</div><span className={`hidden text-xs font-medium sm:block ${step >= index + 1 ? "text-foreground" : "text-muted-foreground"}`}>{title}</span>{index < stepTitles.length - 1 && <div className="h-px flex-1 bg-border"/>}</div>)}</div>{existing && <div className="flex items-center justify-between rounded-xl border border-info/20 bg-info/[0.06] p-3 text-sm"><span>Já existe uma preparação demonstrativa salva.</span><Button variant="ghost" size="sm" onClick={() => onSelect(existing)}>Retomar preparação</Button></div>}{step === 1 && <div className="space-y-5"><div><h2 className="text-xl font-semibold">Dados da hospedagem</h2><p className="mt-1 text-sm text-muted-foreground">Comece pela experiência que será preparada para o hóspede.</p></div><div className="grid gap-4 sm:grid-cols-2"><FormField label={<DemoFieldLabel icon={UserRound}>Nome do hóspede</DemoFieldLabel>}><Input value={guestName} onChange={(event) => setGuestName(event.target.value)} autoFocus placeholder="Ex.: Mariana Oliveira"/></FormField><FormField label={<DemoFieldLabel icon={MessageCircle}>WhatsApp do hóspede</DemoFieldLabel>}><Input value={phone} onChange={(event) => setPhone(formatBrazilianPhone(event.target.value))} inputMode="tel" placeholder="Ex.: (11) 98765-4321"/>{phone && !phoneValid && <p className="mt-1 text-xs text-destructive">Informe um celular com DDD.</p>}</FormField><FormField label={<DemoFieldLabel icon={UsersRound}>Quantidade de hóspedes</DemoFieldLabel>}><Input type="number" min="1" value={guests} onChange={(event) => setGuests(event.target.value)}/></FormField><BrazilianDateTimeField label="Check-in" value={checkIn} onChange={setCheckIn}/><BrazilianDateTimeField label="Check-out" value={checkOut} onChange={setCheckOut} rangeError={checkIn && checkOut && !isCheckoutAfterCheckin(checkIn, checkOut) ? "O check-out deve ser posterior ao check-in." : undefined}/></div><FormField label={<DemoFieldLabel icon={House}>Unidade</DemoFieldLabel>}><Select value={unitId} onChange={(event) => { setUnitId(event.target.value); setAccessType(event.target.value === "studio-vila-nova" ? "yale" : "ekaza"); }}><option value="studio-vila-nova">Studio Vila Nova</option><option value="apartamento-demo-zigbee">Apartamento Demo Zigbee</option></Select></FormField><FormField label={<DemoFieldLabel icon={NotebookPen}>Observações</DemoFieldLabel>}><Input value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Ex.: Chegada após 22h ou necessita berço"/></FormField><Button className="w-full sm:w-auto" disabled={!guestName || !phoneValid || !isCheckoutAfterCheckin(checkIn, checkOut)} onClick={() => setStep(2)}>Continuar</Button></div>}{step === 2 && <div className="space-y-5"><div><h2 className="text-xl font-semibold">Escolha da fechadura</h2><p className="mt-1 text-sm text-muted-foreground">Escolha a experiência de acesso que será demonstrada.</p></div><div className="grid gap-4 md:grid-cols-3"><button type="button" onClick={() => selectAccess("yale")} className={`rounded-2xl border p-6 text-left transition ${accessType === "yale" ? "border-primary bg-primary/[0.06] shadow-sm" : "hover:border-primary/40"}`}><p className="text-lg font-semibold">Yale</p><p className="mt-2 text-sm text-muted-foreground">Fluxo assistido. O sistema solicitará o PIN criado no Yale Hub Connect.</p><span className="mt-5 inline-flex rounded-full bg-surface px-3 py-1 text-sm font-medium">Selecionar</span></button><button type="button" onClick={() => selectAccess("ekaza")} className={`rounded-2xl border p-6 text-left transition ${accessType === "ekaza" ? "border-primary bg-primary/[0.06] shadow-sm" : "hover:border-primary/40"}`}><p className="text-lg font-semibold">Ekaza</p><p className="mt-2 text-sm text-muted-foreground">Fluxo automático. O sistema gerará automaticamente a chave digital.</p><span className="mt-5 inline-flex rounded-full bg-surface px-3 py-1 text-sm font-medium">Selecionar</span></button><div className="rounded-2xl border border-dashed bg-muted/30 p-6 text-left opacity-70"><p className="text-lg font-semibold">Akubela</p><p className="mt-2 text-sm text-muted-foreground">Nova experiência de acesso planejada para uma próxima etapa.</p><span className="mt-5 inline-flex rounded-full bg-surface px-3 py-1 text-sm font-medium">Em breve</span></div></div><div className="flex gap-2"><Button variant="ghost" onClick={() => setStep(1)}>Voltar</Button><Button onClick={() => setStep(3)}>Continuar</Button></div></div>}{step === 3 && <div className="space-y-5"><div><h2 className="text-xl font-semibold">Resumo</h2><p className="mt-1 text-sm text-muted-foreground">Confira os dados antes de configurar o acesso.</p></div><div className="rounded-2xl border bg-surface p-5"><div className="grid gap-4 sm:grid-cols-2">{[["Hóspede", guestName], ["WhatsApp", phone], ["Unidade", selectedUnit.unitName], ["Check-in", formatLocalDate(checkIn)], ["Check-out", formatLocalDate(checkOut)], ["Hóspedes", guests || "1"], ["Tipo de acesso", accessType === "yale" ? "Yale · assistido" : "Ekaza · automático"]].map(([label, value]) => <div key={label}><p className="text-xs text-muted-foreground">{label}</p><p className="mt-1 font-medium">{value}</p></div>)}</div><p className="mt-5 border-t pt-4 text-sm text-muted-foreground">{nights} noite{nights > 1 ? "s" : ""} · A jornada será preparada localmente, sem integração externa.</p></div><div className="flex gap-2"><Button variant="ghost" onClick={() => setStep(2)}>Voltar</Button><Button className="flex-1 sm:flex-none" onClick={create}>Configurar acesso</Button></div></div>}<Button variant="ghost" className="w-full" onClick={onClose}>Cancelar</Button></div></Modal>;
}
function ManualExperienceCard({ reservation, onClose, onUpdate, onSuccess, onDemoCompleted, accessOpen, onAccessOpenChange }: {
    reservation: ManualAirbnbReservation;
    onClose: () => void;
    onUpdate: () => void;
    onSuccess: (type: DemoSuccessState) => void;
    ...
    const isStudioVilaNova = isStudioVilaNovaJourney(reservation.unitId);
    const requiresFNRH = demoReservationRequiresFNRH(reservation.unitId, reservation.requiresFNRH);
    const preparationCompleted = !requiresFNRH || Boolean(reservation.fnrhCompleted);
    const readyForYale = Boolean(reservation.preCheckinCompleted && preparationCompleted);
    ...
    function completePreCheckin() { updateManualAirbnbPreparation(reservation.id, { preCheckinCompleted: true }, [reservation.lockMode === "automatic-demo" ? "Pré-check-in iniciado" : "Pré-check-in concluído"]); onUpdate(); showToast(isStudioVilaNova ? "Preparação iniciada." : "Pré-check-in concluído."); if (!requiresFNRH && reservation.lockMode === "automatic-demo") void prepareAccess(); }
    ...
    function confirmYalePin() { const cleanPin = yalePin.replace(/\D/g, ""); if (!cleanPin)
        return; setIsConfirmingPin(true); window.setTimeout(() => { if (isStudioVilaNova) updateManualAirbnbPreparation(reservation.id, { preCheckinCompleted: true }, ["Preparação interna concluída"]); setDemoAccessSecret(reservation.id, cleanPin); updateManualAirbnbAccess(reservation.id, { pinMasked: `${cleanPin.slice(0, 2)}${"•".repeat(Math.max(4, cleanPin.length - 2))}`, accessStatus: "active", accessPrepared: true }, ["PIN Yale configurado", "QR Code criado", "Portal atualizado"]); setYalePin(""); setIsConfirmingPin(false); setAccessOpen(false); onUpdate(); if (isStudioVilaNova) setSendOpen(true); else onSuccess("preparation"); }, 650); }
    ...
    function sendExperience() { if (!getDemoAccessSecret(reservation.id)) {
        showToast("O acesso temporário ainda não foi preparado.");
        return;
    } sendManualAirbnbExperience(reservation.id); onUpdate(); setSendOpen(false); setAccessOpen(false); if (isStudioVilaNova) onSuccess("studio-message"); else { onDemoCompleted?.(); onSuccess("message"); } }
    ...
    const studioJourney = isStudioVilaNova ? getStudioVilaNovaJourney(reservation) : null;
    const checklist = studioJourney
        ? studioJourney.steps.map(({ label, completed }) => [label, completed] as const)
        : [["Reserva confirmada", true], ["Unidade definida", true], ["Pré-check-in", Boolean(reservation.preCheckinCompleted)], ...(requiresFNRH ? [["FNRH Digital", Boolean(reservation.fnrhCompleted)] as const] : []), ["Portal do Hóspede", Boolean(reservation.accessPrepared)], [isYale ? "PIN Yale" : reservation.accessStatus === "generating" ? "Gerando acesso" : "Chave Digital", Boolean(reservation.accessPrepared)], ["QR Code", Boolean(reservation.accessPrepared)], ["Mensagem", Boolean(reservation.accessPrepared)]] as const;
    ...
    const nextStep = studioJourney?.nextStep ?? (reservation.sent ? "🎉 Hospedagem preparada e enviada ao hóspede" : !reservation.preCheckinCompleted ? "Iniciar pré-check-in" : requiresFNRH && !reservation.fnrhCompleted ? "Concluir FNRH Digital" : isYale && !reservation.accessPrepared ? "Aguardando PIN Yale" : reservation.accessStatus === "generating" ? "Gerando acesso automaticamente" : "Confirmar envio ao hóspede");
    const journeySteps = studioJourney?.steps.map((step) => step.label) ?? ["Reserva", "Pré-check-in", ...(requiresFNRH ? ["FNRH"] : []), isYale ? "PIN Yale" : "Acesso automático", "Portal", "QR", "Mensagem", "Envio"];
    const completionItems = studioJourney ? studioJourney.steps.filter((step) => step.completed).map((step) => step.label) : ["Reserva confirmada", "Pré-check-in concluído", ...(requiresFNRH ? ["FNRH registrada"] : []), `Acesso ${isYale ? "Yale" : "Ekaza"} preparado`, "Portal do Hóspede criado", "QR Code criado", "Mensagem preparada"];
    return <Card className={`overflow-hidden border-border/70 shadow-sm ${reservation.accessPrepared ? "border-success/30" : reservation.isGuidedDemo ? "border-accent/40" : ""}`}><CardHeader className="bg-gradient-to-br from-primary/[0.08] via-surface to-accent/[0.08]"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-primary">Centro de preparação da hospedagem</p><CardTitle className="mt-2 text-2xl">{reservation.sent ? "🎉 Hospedagem preparada e enviada ao hóspede" : reservation.accessPrepared ? "Hospedagem pronta para envio" : "Preparando a chegada"}</CardTitle><CardDescription className="mt-1">{reservation.guestName} · {reservation.unitName || "Studio Vila Nova"} · {formatLocalDate(reservation.checkIn)}</CardDescription></div><div className={`rounded-full px-3 py-1.5 text-sm font-semibold ${reservation.accessPrepared ? "bg-success/[0.12] text-success" : "bg-primary/[0.1] text-primary"}`}>{reservation.sent ? "✓ Jornada concluída" : reservation.accessPrepared ? "✓ Preparação concluída" : `${progress}% preparado`}</div></div><div className="mt-5"><div className="flex items-center justify-between text-xs text-muted-foreground"><span>Progresso da jornada</span><span>{completedSteps} de {checklist.length} etapas</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-muted"><div className="h-full rounded-full bg-gradient-to-r from-primary to-accent transition-all duration-700" style={{ width: `${progress}%` }}/></div></div></CardHeader><CardContent className="space-y-6 pt-6"><div><div className="mb-3 flex items-center justify-between"><div><p className="font-semibold">Jornada da hospedagem</p><p className="text-sm text-muted-foreground">O Essencial Stay antecipa as tarefas antes da chegada.</p></div><p className="text-sm font-medium text-primary">{nextStep}</p></div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{checklist.map(([label, completed]) => <div key={label} className={`rounded-xl border p-4 transition-all duration-300 ${completed ? "border-success/20 bg-success/[0.05]" : label === "Gerando acesso" ? "border-info/30 bg-info/[0.05]" : "bg-surface"}`}><p className={`text-lg ${completed ? "text-success" : label === "Gerando acesso" ? "text-info" : "text-muted-foreground"}`}>{completed ? "✓" : label === "Gerando acesso" ? "⏳" : "○"}</p><p className="mt-2 text-sm font-semibold">{label}</p><p className="mt-1 text-xs text-muted-foreground">{completed ? "Preparado" : label === "Gerando acesso" ? "Em andamento" : "Aguardando etapa anterior"}</p></div>)}</div></div><div className="relative grid gap-2 rounded-xl border bg-surface p-4 sm:grid-cols-4">{journeySteps.map((step, index) => <div key={step} className="flex items-center gap-2 text-xs"><span className={`flex size-6 shrink-0 items-center justify-center rounded-full ${index < journeySteps.length - 1 ? index < Math.ceil((progress / 100) * (journeySteps.length - 1)) ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground" : reservation.sent ? "bg-success text-white" : "bg-muted text-muted-foreground"}`}>{index + 1}</span><span className="font-medium">{step}</span></div>)}</div><div className="rounded-xl border bg-surface p-4"><p className="text-sm font-semibold">Atividade recente</p><p className="mt-2 text-sm text-muted-foreground">{timeline.join(" · ") || "Reserva confirmada"}</p></div><div className="flex flex-wrap gap-2"><Button onClick={() => window.open(reservation.portalUrl, "_blank", "noopener,noreferrer")}>Visualizar Portal</Button>{!reservation.sent && <Button variant="outline" disabled={!reservation.accessPrepared} onClick={openWhatsApp}>Enviar ao hóspede</Button>}<Button variant="ghost" onClick={() => setAccessOpen(true)}>Ver preparação</Button>{reservation.sent && <Button variant="ghost" onClick={() => window.location.reload()}>Nova demonstração</Button>}<Button variant="ghost" onClick={onClose}>Fechar</Button></div><Modal open={isAccessOpen} title={isStudioVilaNova && !reservation.accessPrepared ? "Configurar PIN Yale" : reservation.accessPrepared ? "🎉 Hospedagem preparada com sucesso" : "Preparar hospedagem"} description={isStudioVilaNova && !reservation.accessPrepared ? "Informe o PIN criado no Yale Hub Connect para preparar o acesso do hóspede." : reservation.accessPrepared ? "Toda a jornada foi preparada pelo Essencial Stay. Agora basta confirmar o envio das informações ao hóspede." : "Simulação local e segura para a jornada Airbnb."} onClose={() => setAccessOpen(false)}><div className="space-y-4 text-sm"><div className="rounded-md bg-surface p-4"><p className="font-semibold">{reservation.guestName} · {reservation.unitName || "Studio Vila Nova"}</p><p className="mt-1">Validade: {formatLocalDate(reservation.checkIn)} até {formatLocalDate(reservation.checkOut)}</p></div>{!reservation.preCheckinCompleted && <Button className="w-full" onClick={completePreCheckin}>{isStudioVilaNova ? "Continuar para configurar PIN" : "Concluir pré-check-in"}</Button>}{requiresFNRH && reservation.preCheckinCompleted && !reservation.fnrhCompleted && <Button className="w-full" onClick={completeFnrh}>Concluir FNRH digital</Button>}{requiresFNRH && reservation.fnrhCompleted && <div className="rounded-md border border-success/20 bg-success/[0.08] p-3"><p className="font-semibold text-success">✓ FNRH recebida</p><p className="mt-1 font-medium">📲 FNRH compartilhada com o anfitrião</p><p className="mt-1 text-muted-foreground">A ficha foi registrada e a preparação do acesso continuará automaticamente.</p></div>}{isYale && readyForYale && !reservation.accessPrepared && <div className="rounded-md border border-accent/30 bg-accent/5 p-4"><p className="font-semibold">Configurar PIN Yale</p><p className="mt-1 text-muted-foreground">Digite o PIN criado no Hub Connect. O código não será persistido em texto aberto.</p><Input className="mt-3" type="password" inputMode="numeric" value={yalePin} onChange={(event) => setYalePin(event.target.value.replace(/\D/g, ""))} placeholder="PIN Yale"/><Button className="mt-3 w-full" disabled={!yalePin || isConfirmingPin} onClick={confirmYalePin}>{isConfirmingPin ? "Preparando hospedagem..." : "Confirmar PIN"}</Button></div>}{!isYale && preparationCompleted && !reservation.accessPrepared && <p className="rounded-md bg-info/[0.1] p-3 text-info">Gerando acesso automaticamente...</p>}{reservation.accessPrepared && <><div className="space-y-2 rounded-xl border border-success/20 bg-success/[0.05] p-4">{completionItems.map((item) => <p key={item} className="flex items-center gap-2 font-medium text-success"><Check className="size-4"/>{item}</p>)}</div>{!reservation.sent && <Button className="w-full text-base" size="lg" onClick={openWhatsApp}>📲 Enviar ao hóspede pelo WhatsApp</Button>}<Button className="w-full" variant="outline" onClick={() => window.open(reservation.portalUrl, "_blank", "noopener,noreferrer")}>Visualizar Portal do Hóspede</Button></>}</div></Modal><Modal open={sendOpen} title={isStudioVilaNova ? "Mensagem ao hóspede" : "WhatsApp demonstrativo"} description={isStudioVilaNova ? "Confira todas as informações da experiência antes de simular o envio." : "Pré-visualização da mensagem que será enviada ao hóspede. Nenhuma integração externa será utilizada."} onClose={() => setSendOpen(false)} size={isStudioVilaNova ? "large" : "medium"}><WhatsAppMessagePreview reservation={reservation} accessCode={accessCode || ""} onCopy={() => { void navigator.clipboard?.writeText(message); showToast("Mensagem copiada"); }} onSend={sendExperience}/></Modal></CardContent></Card>;
}

function DemoSuccessFeedback({ type, onClose }: { type: DemoSuccessState; onClose: () => void }) {
    const preparation = type === "preparation";
    const items = preparation ? ["Reserva confirmada", "PIN configurado", "QR Code preparado", "Portal criado", "Mensagem pronta"] : [];
    if (type === "studio-message") return <Modal open title="Mensagem enviada com sucesso!" description="As informações da hospedagem foram preparadas e enviadas ao hóspede." onClose={onClose} size="large"><div className="space-y-6"><div className="grid gap-3 sm:grid-cols-2">{["PIN temporário", "Portal do Hóspede", "QR Code", "Regras da hospedagem", "Automação disponível", "Dicas da região"].map((item) => <div key={item} className="flex items-center gap-3 rounded-xl border border-success/20 bg-success/[0.06] p-4 font-medium text-success"><CheckCircle2 className="size-5 shrink-0"/>{item}</div>)}</div><p className="rounded-xl bg-surface p-4 text-center font-semibold">Agora o hóspede já pode iniciar sua experiência digital.</p><Button className="w-full" size="lg" onClick={onClose}>Concluir demonstração</Button></div></Modal>;
    return <div className="fixed inset-x-4 top-5 z-[70] animate-slide-up sm:left-auto sm:right-5 sm:w-[30rem]" role="status" aria-live="polite"><Card className="border-success/30 bg-card shadow-floating"><CardContent className="p-5"><div className="flex items-start gap-3"><div className="grid size-10 shrink-0 place-items-center rounded-full bg-success/[0.12] text-success"><CheckCircle2 className="size-5"/></div><div className="min-w-0 flex-1"><h2 className="text-lg font-semibold">{preparation ? "🎉 Hospedagem preparada com sucesso!" : "Mensagem enviada com sucesso!"}</h2><p className="mt-1 text-sm text-muted-foreground">{preparation ? "A jornada digital do hóspede foi preparada com sucesso." : "O hóspede recebeu o acesso, o QR Code e o link do Portal do Hóspede. A hospedagem está pronta para começar."}</p>{preparation && <div className="mt-4 grid gap-2 text-sm">{items.map((item) => <p key={item} className="flex items-center gap-2 font-medium text-success"><Check className="size-4"/>{item}</p>)}</div>}</div><Button variant="ghost" size="icon" className="size-8" onClick={onClose} aria-label="Fechar confirmação"><X className="size-4"/></Button></div></CardContent></Card></div>;
}
    ...
    return <div className="space-y-5 text-sm">
        <div className="grid gap-4 sm:grid-cols-2">
            <section className="rounded-2xl border bg-card p-5 shadow-sm" aria-labelledby="whatsapp-recipient-title">
                ...
                <p className="mt-2 flex items-center gap-2 text-muted-foreground"><MessageCircle className="size-4" aria-hidden="true"/><span><span className="block text-xs">WhatsApp</span>{reservation.phone}</span></p>
            </section>
        ...
        <section className="space-y-4 rounded-2xl border bg-surface/70 p-5 sm:p-6" aria-label="Mensagem pronta para envio">
            ...
            <div className="rounded-xl border border-info/20 bg-info/[0.06] p-4"><p className="flex items-center gap-2 font-semibold"><ExternalLink className="size-4 text-info" aria-hidden="true"/>Portal do Hóspede</p><a className="mt-2 block break-words text-sm font-medium text-primary underline-offset-4 [overflow-wrap:anywhere] hover:underline focus-visible:rounded focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" href={reservation.portalUrl} target="_blank" rel="noreferrer">{reservation.portalUrl}</a></div>
            <div className="rounded-xl border border-accent/25 bg-accent/[0.08] p-4"><p className="flex items-center gap-2 font-semibold"><KeyRound className="size-4 text-accent" aria-hidden="true"/>Acesso temporário</p><p className="mt-3 font-mono text-2xl font-bold tracking-[0.04em]">{accessCode}</p></div>
            {isStudioVilaNova && <div className="grid gap-3 sm:grid-cols-2"><div className="rounded-xl border bg-card p-4"><p className="flex items-center gap-2 font-semibold"><QrCode className="size-5 text-primary"/>QR Code</p><p className="mt-2 text-sm text-muted-foreground">Preparado para abrir o Portal do Hóspede com segurança.</p></div><div className="rounded-xl border bg-card p-4"><p className="flex items-center gap-2 font-semibold"><ShieldCheck className="size-5 text-success"/>Regras da hospedagem</p><p className="mt-2 text-sm text-muted-foreground">Horários, orientações de acesso e cuidados com o Studio.</p></div><div className="rounded-xl border bg-card p-4"><p className="flex items-center gap-2 font-semibold"><Sparkles className="size-5 text-accent"/>Automação disponível</p><p className="mt-2 text-sm text-muted-foreground">Iluminação e cena de boas-vindas disponíveis no portal.</p></div><div className="rounded-xl border bg-card p-4"><p className="flex items-center gap-2 font-semibold"><MapPin className="size-5 text-info"/>Dicas da região</p><p className="mt-2 text-sm text-muted-foreground">Restaurantes, transporte e experiências próximas ao Studio.</p></div></div>}
...
function NewStayModal({ open, step, form, onForm, onStep, onClose, onCreate }: {
    ...
    {step === 1 && <div className="grid gap-4 sm:grid-cols-2"><FormField label="Nome completo"><Input value={form.name} onChange={(e) => set("name", e.target.value)} placeholder="Hóspede fictício"/></FormField><FormField label="Telefone"><Input value={form.phone} onChange={(e) => set("phone", e.target.value)} placeholder="(00) 00000-0000"/></FormField><FormField label="E-mail"><Input type="email" value={form.email} onChange={(e) => set("email", e.target.value)} placeholder="nome@example.com"/></FormField><FormField label={<DemoFieldLabel icon={UsersRound}>Quantidade de hóspedes</DemoFieldLabel>}><Input type="number" min="1" value={form.partySize} onChange={(e) => set("partySize", e.target.value)}/></FormField></div>}
    {step === 2 && <div className="grid gap-4 sm:grid-cols-2"><FormField label="Propriedade"><Input value="Hotel Summit Monaco" disabled/></FormField><FormField label={<DemoFieldLabel icon={House}>Unidade</DemoFieldLabel>}><Select value={form.unit} onChange={(e) => set("unit", e.target.value)}><option>Apartamento 901</option><option>Apartamento 508</option><option>Apartamento 704</option></Select></FormField><BrazilianDateTimeField label="Check-in" value={form.checkIn} onChange={(value) => set("checkIn", value)}/><BrazilianDateTimeField label="Check-out" value={form.checkOut} onChange={(value) => set("checkOut", value)} rangeError={form.checkIn && form.checkOut && !isCheckoutAfterCheckin(form.checkIn, form.checkOut) ? "O check-out deve ser posterior ao check-in." : undefined}/><FormField label={<DemoFieldLabel icon={NotebookPen}>Observações</DemoFieldLabel>} optional className="sm:col-span-2"><Input value={form.notes} onChange={(e) => set("notes", e.target.value)}/></FormField></div>}
    ...
    <div className="flex justify-between border-t pt-5"><Button variant="ghost" onClick={step === 1 ? onClose : () => onStep(step - 1)}>{step === 1 ? "Cancelar" : "Voltar"}</Button>{step < 5 ? <Button variant="accent" disabled={step === 2 && !isCheckoutAfterCheckin(form.checkIn, form.checkOut)} onClick={() => onStep(step + 1)}>Continuar <ArrowRight className="size-4"/></Button> : <Button variant="accent" onClick={onCreate}><Sparkles className="size-4"/>Criar hospedagem e preparar experiência</Button>}</div>
  </div></Modal>;
}
...
function isoLocalToVisual(value: string) { if (!value) return { date: "", time: "" }; const [date, time = ""] = value.split("T"); const [year, month, day] = date.split("-"); return { date: day && month && year ? `${day}/${month}/${year}` : "", time: time.slice(0, 5) }; }
function formatBrazilianDateInput(value: string) { const digits = value.replace(/\D/g, "").slice(0, 8); return [digits.slice(0, 2), digits.slice(2, 4), digits.slice(4, 8)].filter(Boolean).join("/"); }
function format24HourTimeInput(value: string) { const digits = value.replace(/\D/g, "").slice(0, 4); return digits.length > 2 ? `${digits.slice(0, 2)}:${digits.slice(2)}` : digits; }
function isValidBrazilianDate(value: string) { const match = /^(\d{2})\/(\d{2})\/(\d{4})$/.exec(value); if (!match) return false; const day = Number(match[1]); const month = Number(match[2]); const year = Number(match[3]); if (year < 1000 || month < 1 || month > 12) return false; const daysInMonth = new Date(year, month, 0).getDate(); return day >= 1 && day <= daysInMonth; }
function isValid24HourTime(value: string) { const match = /^(\d{2}):(\d{2})$/.exec(value); return Boolean(match && Number(match[1]) <= 23 && Number(match[2]) <= 59); }
function visualDateTimeToIsoLocal(date: string, time: string) { const [day, month, year] = date.split("/"); return `${year}-${month}-${day}T${time}`; }
function isValidIsoLocalDateTime(value: string) { const visual = isoLocalToVisual(value); return isValidBrazilianDate(visual.date) && isValid24HourTime(visual.time); }
function isCheckoutAfterCheckin(checkIn: string, checkOut: string) { return isValidIsoLocalDateTime(checkIn) && isValidIsoLocalDateTime(checkOut) && checkOut > checkIn; }
