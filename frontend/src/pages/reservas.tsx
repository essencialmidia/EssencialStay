...
function AirbnbReservationModal({ open, organizationId, onClose, onCreated }: {
    ...
    return <Modal open={open} title="Preparar nova hospedagem" description="Preencha as informações da reserva para iniciar a jornada do hóspede." onClose={onClose}>{step === 1 ? <div className="space-y-3"><FormField label="👤 Nome do hóspede *"><Input value={name} onChange={(event) => setName(event.target.value)} autoFocus placeholder="Ex.: Mariana Oliveira"/></FormField><FormField label="📱 WhatsApp do hóspede *"><Input value={phone} onChange={(event) => setPhone(formatBrazilianPhone(event.target.value))} inputMode="tel" placeholder="Ex.: (11) 98765-4321"/>{phone && !phoneValid && <p className="mt-1 text-xs text-destructive">Informe um celular com DDD.</p>}</FormField><FormField label="🏡 Unidade"><Select value={unitId} onChange={(event) => setUnitId(event.target.value)}>{Object.values(demoLockConfigurations).map((item) => <option key={item.id} value={item.id}>🏡 {item.unitName}</option>)}</Select></FormField><div className="rounded-md border bg-surface p-3 text-sm"><p className="font-semibold">{lock.displayName} — {lock.mode === "assisted" ? "modo assistido" : "modo demonstração"}</p>{lock.mode === "assisted" ? <p className="mt-1 text-muted-foreground">O PIN Yale será informado somente após o pré-check-in e a FNRH. Nenhuma integração Yale é usada.</p> : <p className="mt-1 text-muted-foreground">O acesso será gerado por um provider local de demonstração.</p>}</div><div className="grid grid-cols-2 gap-3"><FormField label="📅 Check-in *"><Input type="datetime-local" lang="pt-BR" value={checkIn} onChange={(event) => setCheckIn(event.target.value)}/></FormField><FormField label="📅 Check-out *"><Input type="datetime-local" lang="pt-BR" value={checkOut} onChange={(event) => setCheckOut(event.target.value)}/></FormField></div><div className="grid grid-cols-2 gap-3"><FormField label="👥 Hóspedes"><Input type="number" min="1" value={guests} onChange={(event) => setGuests(event.target.value)}/></FormField><FormField label="Origem"><Select value={source} onChange={(event) => setSource(event.target.value as ManualAirbnbReservation["source"])}><option>Airbnb</option><option>Booking</option><option>Direta</option><option>Outro</option></Select></FormField></div><FormField label="📝 Observações"><Input value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Ex.: Chegada após 22h ou necessita berço"/></FormField><Button className="w-full" disabled={!name || !phoneValid} onClick={() => setStep(2)}>Continuar →</Button></div> : <div className="space-y-4"><div className="rounded-md bg-surface p-4 text-sm"><p className="font-semibold">{name} · {lock.unitName}</p><p>{nights} noites · {source}</p><p>{formatLocalDate(checkIn)} → {formatLocalDate(checkOut)}</p></div><Button className="w-full" onClick={create}>Preparar hospedagem</Button><Button className="w-full" variant="ghost" onClick={() => setStep(1)}>Voltar</Button></div>}</Modal>;
}
...
function NewStayModal({ open, step, form, onForm, onStep, onClose, onCreate }: {
    ...
    {step === 2 && <div className="grid gap-4 sm:grid-cols-2"><FormField label="Propriedade"><Input value="Hotel Summit Monaco" disabled/></FormField><FormField label="🏡 Unidade"><Select value={form.unit} onChange={(e) => set("unit", e.target.value)}><option>Apartamento 901</option><option>Apartamento 508</option><option>Apartamento 704</option></Select></FormField><FormField label="📅 Check-in"><Input type="datetime-local" lang="pt-BR" value={form.checkIn} onChange={(e) => set("checkIn", e.target.value)}/></FormField><FormField label="Checkout"><Input type="datetime-local" lang="pt-BR" value={form.checkOut} onChange={(e) => set("checkOut", e.target.value)}/></FormField><FormField label="📝 Observações" optional className="sm:col-span-2"><Input value={form.notes} onChange={(e) => set("notes", e.target.value)}/></FormField></div>}
