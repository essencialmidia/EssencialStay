...
export function AdminCompanyDetailsPage() {
  ...
  return <div className="min-w-0 space-y-7"><PageHeader title={empresa.nome_fantasia || empresa.nome} description={empresa.nome} badge={empresa.status === "ativo" ? "Ativa" : empresa.status} breadcrumb={["Administração", "Empresas", empresa.nome_fantasia || empresa.nome]} actions={<><Button variant="outline" onClick={enterOperation}>Acessar painel da empresa</Button>{canManagePlatform && <Button onClick={() => navigate(`/admin/empresas/${empresa.id}/propriedades/nova`)}><Plus className="size-4" />Adicionar propriedade</Button>}</>} />
    ...
    <Modal open={editing} size="large" title="Editar empresa cliente" description="Atualize os dados cadastrais sem alterar o acesso da empresa." onClose={() => setEditing(false)}><OrganizationForm value={empresa} onSubmit={save} submitting={submitting} onCancel={() => setEditing(false)} /></Modal>
  </div>;
}
