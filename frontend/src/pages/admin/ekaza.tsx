...
type Selection = { details: DeviceDetails; status: DeviceStatus; specifications: { functions: Capability[]; status: Capability[] } };
...
export function AdminEkazaPage() {
  ...
  async function viewDetails(device: EkazaDevice) {
    const result = await run("details", async () => { const path = `/api/v1/integrations/ekaza/devices/${encodeURIComponent(device.providerDeviceId)}`; const [details, status, specifications] = await Promise.all([getJson<DeviceDetails>(path, administrativeKey), getJson<DeviceStatus>(`${path}/status`, administrativeKey), getJson<Selection["specifications"]>(`${path}/specifications`, administrativeKey)]); return { details, status, specifications }; });
    if (result) { setTab("info"); setSelection(result); }
  }
