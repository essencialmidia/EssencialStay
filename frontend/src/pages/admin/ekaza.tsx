...
const apiBaseUrl = import.meta.env.VITE_EKAZA_API_BASE_URL?.trim().replace(/\/+$/, "") ?? "";
const adminApiKey = import.meta.env.VITE_EKAZA_ADMIN_API_KEY?.trim() ?? "";

function apiUrl(path: string) {
  return `${apiBaseUrl}${path}`;
}

async function getJson<T>(path: string, withAdminKey = false): Promise<T> {
  const response = await fetch(apiUrl(path), {
    headers: withAdminKey ? { "x-ekaza-admin-key": adminApiKey } : undefined,
  });
...
export function AdminEkazaPage() {
  ...
  async function listDevices() {
    ...
    setError(null);
    try {
      const result = await getJson<DevicesResponse>("/api/v1/integrations/ekaza/devices", true);
  ...
  return <div className="space-y-6">
    ...
    <Card>
      ...
      <CardContent className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3"><div className="grid size-10 place-items-center rounded-md bg-highlight/[0.14] text-highlight-foreground"><PlugZap className="size-5" /></div><p className="text-sm text-muted-foreground">A requisição envia o cabeçalho administrativo configurado no frontend.</p></div>
